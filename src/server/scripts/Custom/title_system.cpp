	...
	public:
		...
		struct npc_titleAI : public ScriptedAI
        {
			...
			void MoveInLineOfSight(Unit* pWho)
			{
				...
				//Player* player = pWho->GetName;

				if (pWho->GetTypeId() == TYPEID_PLAYER && me->IsWithinDistInMap(pWho, 10.0f) && me->IsWithinLOSInMap(pWho) && hasidolized == false /*pWho->getFaction()== ALLIANCE &&*/ && pWho->GetName() == "Trifuse" )
				{
					if(Creature * c = me->FindNearestCreature(500150, 20.0f, true))
					c->HandleEmoteCommand(EMOTE_ONESHOT_KNEEL);

					if(Creature * c = me->FindNearestCreature(500147, 20.0f, true))
					c->HandleEmoteCommand(EMOTE_ONESHOT_KNEEL);

					if(Creature * c = me->FindNearestCreature(500146, 20.0f, true))
					c->HandleEmoteCommand(EMOTE_ONESHOT_BOW);

					if(Creature * c = me->FindNearestCreature(19859, 20.0f, true))
					c->HandleEmoteCommand(EMOTE_ONESHOT_KNEEL);

					if(Creature * c = me->FindNearestCreature(500164, 20.0f, true))
					c->HandleEmoteCommand(EMOTE_ONESHOT_BOW);

					if(Creature * c = me->FindNearestCreature(900005, 20.0f, true))
					c->HandleEmoteCommand(EMOTE_ONESHOT_KNEEL);
			...
			{
				if (resettimer <= diff && hasidolized == true)
				{
					hasidolized = false;
