...
#define NPC_PRINCE    14435
#define GOSSIP_DEMITRIAN1 "What do you know of it?"
#define GOSSIP_DEMITRIAN2 "What must be done?"
#define GOSSIP_DEMITRIAN3 "The Firelord and all who dare stand in my way shall reel from my wrath."
#define GOSSIP_DEMITRIAN4 "I shall scour the earth for this Earthshaper. What of the essence?"
#define GOSSIP_DEMITRIAN5 "Give me the vessel, Highlord."
...
public:
    ...
    case GOSSIP_ACTION_INFO_DEF:
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_DEMITRIAN3, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
        player->SEND_GOSSIP_MENU(6955, creature->GetGUID());
        break;
    case GOSSIP_ACTION_INFO_DEF+1:
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_DEMITRIAN4, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
        player->SEND_GOSSIP_MENU(6956, creature->GetGUID());
        break;
    case GOSSIP_ACTION_INFO_DEF+2:
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_DEMITRIAN5, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+3);
        player->SEND_GOSSIP_MENU(6958, creature->GetGUID());
        break;
    /*case GOSSIP_ACTION_INFO_DEF+4:
        creature->SummonCreature(14435, -6243.06f, 1718.6f, 4.239f, 0.68f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 300000); 
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (player->GetQuestStatus(7786) == QUEST_STATUS_COMPLETE && player->GetQuestStatus(7787) == QUEST_STATUS_NONE && !player->GetQuestStatus(7787) == QUEST_STATUS_COMPLETE && !player->HasItemCount(19018, 1) && !player->HasItemCount(18564, 1))
    {
        player->SEND_GOSSIP_MENU(33000, creature->GetGUID());
        return true;
    }

    if (player->GetQuestStatus(7785) == QUEST_STATUS_NONE && !player->HasItemCount(19016, 1) &&
       (player->HasItemCount(18563, 1, false) || player->HasItemCount(18564, 1, false)))
    {
        if (player->getClass() == CLASS_WARRIOR || player->getClass() == CLASS_PALADIN || player->getClass() == CLASS_HUNTER || player->getClass() == CLASS_ROGUE)
        {
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_DEMITRIAN2, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
        player->SEND_GOSSIP_MENU(6954, creature->GetGUID());
        return true;
        }
    } 
